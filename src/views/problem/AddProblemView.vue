  ...
  axios.get(`/problem`, {
    ...
    .then(function (response) {
      if (response.data.code === 200) {
        ...
        title.value = data.problemTitle;
        answer.value = data.problemAnswer;
      } else {
        console.log('加载问题数据失败!');
      }
    })
    ...
    if(problemId.value==null) {
      ...
      axios.post('/problem/update', {
        ...
        categoryList: category.value,
          ...
          .then(function (response) {
            if (response.data.code === 200) {
              ElMessage({type: 'success', message: '习题修改成功.'})
              router.push("/problems");
            } else {
              ElMessage({type: 'fail', message: '习题修改失败.'})
            }
          })
    }
