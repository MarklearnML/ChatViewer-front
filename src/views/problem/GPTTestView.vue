  ...
  <div class="container">
    <div class="left-container">
      <div class="question-container" v-if="loadingProblem">
        ...
        <el-button round color=#3c813f plain @click="updateProblem" v-if="userStore.token !== '' && isUser" >修改问题</el-button>
        <el-button round color=#3c813f plain @click="deleteProblem" v-if="userStore.token !== '' && isUser" >删除问题</el-button>
  ...
  axios.get('/problem?problemId=' + problemId.value)
      .then(function (response) {
        if (response.data.code === 200) {
          ...
          isUser.value = problem.value.isUser;
          loadAnswer()
        } else {
          console.log('获取问题失败!')
        }
...
function deleteProblem() {
  axios.delete(`/problem/delete?problemId=${problemId.value}`)
      .then(function (response) {
        if (response.data.code === 200) {
...
function updateProblem() {
  router.push(`/problem/add?problemId=${problemId.value}`);
}
