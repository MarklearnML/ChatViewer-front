      ...
      <div class="article-list-region">
        <div v-for="post in posts" :key="post">
          <div class="article-region">
            <DraftCard v-bind="post" class="card"/>
          </div>
        </div>
      </div>
      <!-- 分页器 -->
...
import DraftCard from '@/components/article/DraftCard.vue'
import NavigateBar from "@/components/base/NavigateBar.vue";
import { getCategoryDraftPage } from "@/api/article";
...
function getPage() {
  getCategoryDraftPage(pageIndex.value,pageSize.value).then((res) => {
