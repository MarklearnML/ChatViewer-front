<template>
  ...
  <div class="editor-wrapper" >
    ...
    <div class="editor-submit-block">
      <el-button round class="editor-submit-button" @click="save_article">保存草稿</el-button>
...
onMounted(() => {
  draftId.value = route.query.draftId || null;
  console.info(draftId.value)
  // 检查是否有 draftId
  if (draftId.value) {
    // 创建一个新的 query 对象，移除 draftId
    const newQuery = { ...route.query };
    delete newQuery.draftId;

    // 加载草稿
    getDraft(draftId.value).then((res) => {
      draftId.value = res["articleId"];
      title.value = res["articleTitle"];
      category.value = res["categoryId"];
      abstract.value = res["articleAbstract"];
      vditor.value!.setValue(res["articleContent"]);
      mainPicUrl.value = res["articlePic"];
      if(mainPicUrl.value!="") {
        beforeUploading.value = false;
      }
    })
    // 使用 router.replace 更新 URL，移除 draftId 参数
  ...
  if (userStore["token"] === '') {
    ...
    axios.post('/article/save', {
      ...
      userId: userStore["user"].userId
        ...
        .then(function (response) {
          if (response.data.code === 200) {
            ElMessage({type: 'success', message: '文章保存成功.'})
            router.push({
              name: 'articles',
            })
          } else {
            ElMessage({type: 'fail', message: '文章保存失败.'})
          }
        })
  }
}
